...
def ai_analyze(products_info, context="네이버 쇼핑", api_key=""):
    ...
    prompt = f"""당신은 한국 식품/음료 시장 트렌드 분석 전문가입니다.

아래는 {context}에서 수집한 상위 인기 상품 목록입니다:

{products_info}

다음 항목을 분석해주세요:
1. **맛/플레이버 트렌드**: 상품명에서 드러나는 인기 맛(제로, 복숭아, 레몬 등)과 그 비중
2. **제품 유형 트렌드**: 어떤 종류(탄산, 주스, 차 등)가 주류인지
3. **가격대 분석**: 가격 분포의 특징과 소비자 선호 가격대
4. **브랜드 경쟁 현황**: 주요 브랜드와 시장 포지셔닝
5. **신제품 키워드**: 최근 트렌드를 반영한 핵심 키워드 5개
6. **R&D 인사이트**: 식품 R&D 담당자 관점에서의 제품 개발 시사점

간결하지만 통찰력 있게 분석해주세요. 한국어로 답변하세요."""

...
def ai_analyze(products_info, context="네이버 쇼핑", api_key=""):
    ...
    prompt = f"""
당신은 한국 식품/음료 시장의 데이터 기반 분석 전문가입니다.

데이터 해석 주의사항 (반드시 준수):
- 아래 데이터는 {context}
의 **검색 노출 상품 목록**이며, 시장 전체 판매 데이터가 아님
...
7. **회의용 멘트**
위 분석을 바탕으로 내부 상품기획 회의에서 사용할 수 있는
"1분 브리핑용 회의 멘트"를 작성하세요.

8. **마케팅 기획자 페르소나 분석**
당신은 15년 경력의 음료 마케팅 기획자입니다.
위 데이터를 기반으로 시장 해석과 기획 시사점을 작성하세요.

각 항목에서 데이터의 한계와 해석 조건을 반드시 함께 서술하세요.
한국어로 답변하세요.
"""

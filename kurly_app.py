...
def ai_analyze(products_info, context="네이버 쇼핑", api_key=""):
    ...
    prompt = f"""당신은 한국 식품/음료 시장의 데이터 기반 분석 전문가입니다.

⚠️ 데이터 해석 주의사항 (반드시 준수):
- 아래 데이터는 {context}의 **검색 노출 상품 목록**이며, 시장 전체 판매 데이터가 아님
- 중복 쇼핑몰 등록, 멀티팩, 번들 SKU가 혼재되어 있음
- 따라서 검색 노출 빈도를 소비자 선호도로 과대해석하지 말 것
- "인기", "트렌드", "소비자가 선호" 같은 단정적 표현을 지양할 것

분석 대상 데이터:
{products_info}

다음 항목을 **데이터 한계를 명시하면서** 분석하세요:

1. **고유 SKU 분석**: 중복 등록/멀티팩/번들을 감안하여 실제 고유 제품 수 추정. 동일 제품의 용량·입수 변형이 몇 종인지 구분
2. **입수/용량 보정 가격 분석**: 단품 vs 멀티팩 vs 대용량의 가격 구조. 100ml(또는 100g)당 단가 기준 가격대 분포 추정
3. **판매채널 구조**: 자사몰/오픈마켓/종합몰 등 채널별 등록 패턴. 특정 채널 편중 여부와 그 의미
4. **번들/기획팩 구성 특성**: 묶음 상품 비율과 구성 패턴 (택1, 기획팩, 세트 등). 번들이 많은 카테고리의 의미
5. **맛/플레이버 키워드**: 제품명과 설명에서 추출 가능한 맛 키워드 빈도 (단, 검색 노출 빈도일 뿐 판매 선호와 다를 수 있음을 명시)
6. **R&D 시사점**: 위 분석을 종합한 제품 개발 관점의 관찰 (확정적 제언이 아닌 '검토 가치가 있는 방향' 수준으로)

각 항목에서 데이터의 한계와 해석의 조건을 반드시 함께 서술하세요.
한국어로 답변하세요."""
